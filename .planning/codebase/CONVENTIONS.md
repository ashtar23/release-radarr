...
- camelCase for functions and helpers (`createSoonrApiClient`, `handleSearchRequest`)
...
- Good examples explain why a platform-specific decision exists, such as static Expo env access in [`apps/mobile/src/lib/supabase.ts`](soonr/apps/mobile/src/lib/supabase.ts)
