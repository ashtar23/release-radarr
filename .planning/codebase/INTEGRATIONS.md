...
- RAWG - Game metadata search and detail fallback for backend cache fill
  - Integration method: Deno `fetch` in [`supabase/functions/api/providers/rawg.ts`](soonr/supabase/functions/api/providers/rawg.ts)
...
- Supabase Auth - Email/password sessions for both clients
  ...
  - Token storage:
    ...
    - Mobile: AsyncStorage configured in [`apps/mobile/src/lib/supabase.ts`](soonr/apps/mobile/src/lib/supabase.ts)
...
- Local services are defined in [`supabase/config.toml`](soonr/supabase/config.toml)
