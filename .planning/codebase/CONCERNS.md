...
- Risk: [`supabase/config.toml`](soonr/supabase/config.toml) sets `[functions.api] verify_jwt = false`, which weakens trust in caller identity if carried to non-local environments
