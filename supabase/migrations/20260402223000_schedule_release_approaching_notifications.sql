...
begin
  with candidate_preferences as (
    ...
    where t.earliest_release_date is not null
  ),
  due_candidates as (
    select
      cp.user_id,
...
begin
  ...
  from cron.job
  where jobname = 'generate-release-approaching-notifications';
  ...
  perform cron.schedule(
    'generate-release-approaching-notifications',
    '5 0 * * *',
    $job$select public.generate_release_approaching_notifications(timezone('utc', now())::date);$job$
  );
exception
  when undefined_table then
    raise notice 'pg_cron metadata table not available; skipping notification cron registration';
end;
$$;
