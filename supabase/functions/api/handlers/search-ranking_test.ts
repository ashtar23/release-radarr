...
import {
  getProviderSearchQuery as getProviderSearchQueryForTesting,
  getSearchQueryVariants as getSearchQueryVariantsForTesting,
  inferQuerySearchOptions as inferQuerySearchOptionsForTesting,
...
import {
  getLocalPolicyPageCount as getLocalPolicyPageCountForTesting,
  mergeResults as mergeResultsForTesting,
  rankResultsForTesting,
} from "../utils/search-ranking.ts";
import { getRelevantLocalStaleRatio as getRelevantLocalStaleRatioForTesting } from "../utils/search-freshness-window.ts";
import { evaluateSearchFallbackPolicy } from "../utils/search-fallback-policy.ts";

function getProviderPageBudgetForTesting(
...
function getProviderDecisionReasonForTesting(
...
function getBroadSparseFreshLocalSufficientForTesting(
  intentMode: QueryIntentMode,
  page: number,
  sparseBroadFirstPageLocal: boolean,
  visibleLocalPageCount: number,
...
Deno.test(
  "Ranking V2: canonical title outranks noisy variant for specific query",
  () => {
    ...
    const noisy = createTitleSummary("rawg:noisy", "Prince of Persia Demo", {
      rawgRating: 2.1,
      rawgRatingsCount: 9,
      rawgMetacritic: 24,
      rawgAdded: 80,
...
Deno.test(
  "Ranking V2: quality signal breaks ties between similarly relevant titles",
  () => {
    ...
    const lowerQuality = createTitleSummary("rawg:low", "God of War Ragnarok", {
      rawgRating: 3.2,
    ...
    if (ranked[0]?.id !== highQuality.id) {
      throw new Error(
        `Expected higher quality variant first, got ${ranked[0]?.id}`,
      );
    }
  },
);
...
Deno.test(
  "Ranking V2: low lexical match cannot be rescued by popularity",
  () => {
...
Deno.test(
  "Ranking V2: broad single-token query favors canonical Witcher title",
  () => {
    const query = "witcher";
    const canonical = createTitleSummary(
      "rawg:canon",
...
Deno.test(
  "Ranking V2: broad query promotes canonical title over low-signal exact match",
  () => {
    const query = "crimson";
    const canonical = createTitleSummary(
      "rawg:crimson-desert",
...
Deno.test(
  "Ranking V2: broad astro query demotes non-mobile 'Mobile Astro' mismatch",
  () => {
    const query = "astro";
    const canonical = createTitleSummary("rawg:astro-bot", "Astro Bot", {
      rawgRating: 4.1,
    ...
    const mobileMismatch = createTitleSummary(
      "rawg:mobile-astro",
      "Mobile Astro",
      {
        rawgRating: 4.2,
    ...
    if (ranked[0]?.id !== canonical.id) {
      throw new Error(
        `Expected Astro Bot ahead of Mobile Astro for broad query, got ${
          ranked[0]?.id ?? "none"
        }`,
      );
    }
...
Deno.test(
  "Ranking V2: specific franchise query demotes itch/android noise",
  () => {
...
Deno.test(
  "Freshness window ignores stale tail when top-ranked local page is fresh",
  () => {
    ...
    if (staleRatio > 0.3) {
      throw new Error(
        `Expected visible-window stale ratio <= 0.3, got ${staleRatio}`,
      );
    }
  },
);
...
Deno.test(
  "Broad freshness window follows denoised visible franchise page",
  () => {
    const now = new Date().toISOString();
    const stale = "2020-01-01T00:00:00.000Z";
...
Deno.test(
  "Ranking V2: merged alias query matches canonical Spider-Man title",
  () => {
    ...
    const noisy = createTitleSummary(
      "rawg:spiderman-alpha",
      "Spiderman: Free-Swing Alpha",
      {
        rawgRating: 2.2,
        rawgRatingsCount: 18,
        rawgMetacritic: 35,
...
Deno.test(
  "Ranking V2: broad recency boost favors newer anchored franchise title",
  () => {
    const query = "crimson";
    const futureDate = new Date(Date.now() + 365 * 24 * 60 * 60 * 1000)
      .toISOString()
      .slice(0, 10);
    const upcoming = createTitleSummary(
      "rawg:crimson-desert",
      "Crimson Desert",
      {
    ...
    if (ranked[0]?.id !== upcoming.id) {
      throw new Error(
        `Expected newer anchored Crimson title first, got ${
          ranked[0]?.id ?? "none"
        }`,
      );
    }
...
Deno.test(
  "Ranking V2: broad recency boost does not promote low-signal future noise",
  () => {
    const query = "crimson";
    const futureDate = new Date(Date.now() + 240 * 24 * 60 * 60 * 1000)
      .toISOString()
      .slice(0, 10);
    const established = createTitleSummary(
      "rawg:crimson-echo-stable",
      "Crimson Echo",
      {
    ...
    const futureLowSignal = createTitleSummary(
      "rawg:crimson-quest-noise",
      "Crimson Quest",
      {
        earliestReleaseDate: futureDate,
        rawgRating: 2.2,
    ...
    if (ranked[0]?.id !== established.id) {
      throw new Error(
        `Expected established higher-quality Crimson title first, got ${
          ranked[0]?.id ?? "none"
        }`,
      );
    }
...
Deno.test(
  "Ranking V2: specific franchise query demotes derivative subtitle DLC entries",
  () => {
    const query = "marvel spider man";
    const sequel = createTitleSummary(
      "rawg:spider-man-2",
    ...
    if (!topTwo.includes(sequel.id) || !topTwo.includes(remastered.id)) {
      throw new Error(
        `Expected core Spider-Man titles in top 2, got ${topTwo.join(", ")}`,
      );
    }
    ...
    const derivativeInTopFive = topFive.filter(
      (id) => id === heist.id || id === silverLining.id,
    );
...
Deno.test(
  "Ranking V2: specific top-window cap applies to high-quality derivative subtitle entries",
  () => {
    const query = "marvel spider man";
    const base = createTitleSummary(
      "rawg:spider-man-core",
      "Marvel's Spider-Man",
      {
    ...
    const topFive = ranked.slice(0, 5).map((result) => result.id);
    const derivativeInTopFive = topFive.filter(
      (id) => id === heist.id || id === silverLining.id || id === turfWars.id,
    );
...
Deno.test(
  "Ranking V2: specific derivative intent keeps requested subtitle result high",
  () => {
    const query = "spider man heist";
    const base = createTitleSummary(
      "rawg:spider-man-2018",
      "Marvel's Spider-Man",
      {
        earliestReleaseDate: "2018-09-07",
        rawgRating: 4.7,
    ...
    if (ranked[0]?.id !== heist.id) {
      throw new Error(
        `Expected requested heist subtitle first, got ${ranked[0]?.id ?? "none"}`,
      );
    }
  },
);
...
Deno.test(
  "Ranking V2: parenthetical vendor suffix loses to canonical Wolverine title",
  () => {
    const query = "wolverine";
    const canonical = createTitleSummary(
      "rawg:marvel-wolverine",
...
Deno.test(
  "Search retrieval prefers expanded provider query for merged alias",
  () => {
    const providerQuery = getProviderSearchQueryForTesting("Spiderman");

    if (providerQuery !== "spider man") {
      throw new Error(
        `Expected provider query 'spider man', got '${providerQuery}'`,
      );
    }
  },
);
...
Deno.test(
  "Search retrieval expands alias phrases inside larger sequel queries",
  () => {
    ...
    if (providerQuery !== "spider man 2") {
      throw new Error(
        `Expected provider query 'spider man 2', got '${providerQuery}'`,
      );
    }
  },
);
...
Deno.test(
  "Search retrieval expands adjacent-swap typo variants for single-token queries",
  () => {
    const variants = getSearchQueryVariantsForTesting("Withcer");

    if (!variants.includes("withcer")) {
      throw new Error("Expected original normalized variant 'withcer'");
    }

    if (!variants.includes("witcher")) {
      throw new Error("Expected adjacent-swap typo variant 'witcher'");
    }
  },
);

Deno.test(
  "Search retrieval does not expand adjacent-swap typo variants for short tokens",
  () => {
    const variants = getSearchQueryVariantsForTesting("Astro");

    if (!variants.includes("astro")) {
      throw new Error("Expected normalized variant 'astro'");
    }

    if (variants.some((variant) => variant !== "astro")) {
      throw new Error(
        `Expected no extra typo variants for short token query, got ${variants.join(
          ", ",
        )}`,
      );
    }
...
Deno.test(
  "Search retrieval does not expand adjacent-swap typo variants for multi-token queries",
  () => {
    const variants = getSearchQueryVariantsForTesting("Withcer 3");

    if (!variants.includes("withcer 3")) {
      throw new Error("Expected normalized variant 'withcer 3'");
    }

    if (variants.some((variant) => variant !== "withcer 3")) {
      throw new Error(
        `Expected no adjacent-swap expansion for multi-token queries, got ${variants.join(
          ", ",
        )}`,
      );
    }
...
Deno.test("Sparse broad page-one queries use expanded provider budget", () => {
  ...
  if (sparseBroadBudget !== 3) {
    throw new Error(
      `Expected sparse broad provider budget=3, got ${sparseBroadBudget}`,
    );
  }

  if (healthyBroadBudget !== 1) {
    throw new Error(
      `Expected healthy broad provider budget=1, got ${healthyBroadBudget}`,
    );
  }
  ...
  if (specificBudget !== 1) {
    throw new Error(
      `Expected specific provider budget=1, got ${specificBudget}`,
    );
  }

  if (secondPageBudget !== 1) {
    throw new Error(
      `Expected non-first-page provider budget=1, got ${secondPageBudget}`,
    );
  }
});
...
Deno.test(
  "Fresh sparse broad page can serve local cache without repeated provider refresh",
  () => {
    ...
    const oneStaleInThreeSparseBroad =
      getBroadSparseFreshLocalSufficientForTesting("broad", 1, true, 3, 1 / 3);
    ...
    if (!freshSparseBroad) {
      throw new Error(
        "Expected fresh sparse broad page to be cache-sufficient",
      );
    }
    ...
    if (specificQuery) {
      throw new Error(
        "Expected specific queries not to use sparse-broad cache override",
      );
    }
  },
);
...
Deno.test(
  "Sparse visible broad local page triggers provider fallback even with larger local total",
  () => {
    const query = "wolverine";
    const visibleLegacy = createTitleSummary(
      "rawg:wolverine-legacy",
      "Wolverine",
      {
        rawgRating: 3.6,
    ...
    const lowTailA = createTitleSummary(
      "rawg:wolverine-tail-a",
      "Golfin Wolverine ByMy",
      {
        rawgRating: 1.8,
        rawgRatingsCount: 2,
        rawgMetacritic: 15,
    ...
    const lowTailB = createTitleSummary(
      "rawg:wolverine-tail-b",
      "the wolverine",
      {
        rawgRating: 2.2,
        rawgRatingsCount: 8,
    ...
    if (!policy.needsProvider) {
      throw new Error(
        "Expected provider fallback when visible broad page is sparse",
      );
    }
  },
);
...
Deno.test("Single-token franchise queries are treated as broad intent", () => {
  ...
  if (spiderman.intentMode !== "broad") {
    throw new Error(
      `Expected spiderman to be broad, got ${spiderman.intentMode}`,
    );
  }

  if (wolverine.intentMode !== "broad") {
    throw new Error(
      `Expected wolverine to be broad, got ${wolverine.intentMode}`,
    );
  }
...
Deno.test(
  "Ranking V2: broad franchise query demotes simulator and alpha noise",
  () => {
    const query = "spiderman";
    const canonical = createTitleSummary(
      "rawg:spider-man-2018",
    ...
    const simulator = createTitleSummary(
      "rawg:spiderman-simulator",
      "Spiderman Simulator",
      {
        rawgRating: 2.3,
        rawgRatingsCount: 18,
    ...
    const alpha = createTitleSummary(
      "rawg:spiderman-alpha",
      "Spiderman: Free-Swing Alpha",
      {
        rawgRating: 2.2,
    ...
    if (ranked[0]?.id !== canonical.id) {
      throw new Error(
        `Expected canonical Spider-Man first for broad franchise query, got ${
          ranked[0]?.id ?? "none"
        }`,
      );
    }
...
Deno.test(
  "Ranking V2: broad franchise query demotes vendor suffix and demake noise",
  () => {
    const query = "wolverine";
    const canonical = createTitleSummary(
      "rawg:marvel-wolverine",
    ...
    const demake = createTitleSummary(
      "rawg:wolverine-demake",
      "Wolverine Demake",
      {
        rawgRating: 2.4,
        rawgRatingsCount: 16,
    ...
    if (ranked[0]?.id !== canonical.id) {
      throw new Error(
        `Expected canonical Wolverine first for broad franchise query, got ${
          ranked[0]?.id ?? "none"
        }`,
      );
    }
...
Deno.test(
  "Ranking V2: specific sequel query requires non-numeric franchise match",
  () => {
    ...
    if (ranked[0]?.id !== canonical.id) {
      throw new Error(
        `Expected Spider-Man 2 first for specific sequel query, got ${
          ranked[0]?.id ?? "none"
        }`,
      );
    }
...
Deno.test(
  "Ranking V2: working sequel query still prefers the canonical Witcher title",
  () => {
    ...
    if (ranked[0]?.id !== canonical.id) {
      throw new Error(
        `Expected Witcher 3 first for sequel query, got ${
          ranked[0]?.id ?? "none"
        }`,
      );
    }
...
Deno.test(
  "Ranking V2: apostrophe-less branded query still finds canonical Spider-Man title",
  () => {
    ...
    const olderLiteral = createTitleSummary(
      "rawg:spider-man-2000",
      "Spider-Man (2000)",
      {
        rawgRating: 3.8,
    ...
    if (ranked[0]?.id !== canonical.id) {
      throw new Error(
        `Expected canonical branded Spider-Man title first, got ${
          ranked[0]?.id ?? "none"
        }`,
      );
    }
...
Deno.test(
  "Ranking V2: broad franchise query promotes high-signal branded title over weak literal exact",
  () => {
    const query = "wolverine";
    ...
    const flagship = createTitleSummary(
      "rawg:marvels-wolverine",
      "Marvel's Wolverine",
      {
        rawgRating: 4.8,
        rawgRatingsCount: 3200,
    ...
    if (ranked[0]?.id !== flagship.id) {
      throw new Error(
        `Expected high-signal branded Wolverine title first, got ${
          ranked[0]?.id ?? "none"
        }`,
      );
    }
...
Deno.test(
  "Ranking V2: broad franchise query can elevate a flagship title above legacy literal matches",
  () => {
    const query = "wolverine";
    ...
    const legacy = createTitleSummary(
      "rawg:adamantium",
    ...
    const flagship = createTitleSummary(
      "rawg:marvels-wolverine-flagship",
      "Marvel's Wolverine",
      {
        rawgRating: 4.8,
        rawgRatingsCount: 3200,
...
Deno.test(
  "Broad franchise denoise hides low-quality tail once enough strong results exist",
  () => {
    const query = "wolverine";
    ...
    const origins = createTitleSummary(
      "rawg:origins",
      "X-Men Origins: Wolverine",
      {
        rawgRating: 4.1,
    ...
    if (merged.results.length >= 6) {
      throw new Error(
        `Expected broad franchise denoise to reduce the tail, got ${merged.results.length} results`,
      );
    }
    ...
    if (
      merged.results.some(
        (result: TitleSummary) =>
          result.id === junk.id || result.id === filler.id,
      )
    ) {
      throw new Error(
        "Expected low-quality franchise tail entries to be removed",
      );
    }

    if (
      !merged.results.some((result: TitleSummary) => result.id === flagship.id)
    ) {
      throw new Error(
        "Expected flagship franchise result to remain after denoise",
      );
    }
  },
);
...
Deno.test(
  "Broad franchise denoise falls back to original results when pruning would leave too few entries",
  () => {
    const query = "wolverine";
    const flagship = createTitleSummary(
      "rawg:flagship-min",
...
Deno.test(
  "Broad franchise denoise keeps anticipated mainstream title even when other quality signals are weak",
  () => {
    const query = "wolverine";
    ...
    const anticipated = createTitleSummary(
      "rawg:anticipated",
      "Marvel's Wolverine",
      {
        rawgRating: null,
    ...
    const junk = createTitleSummary(
      "rawg:junk-protected-check",
      "Golfin Wolverine ByMy",
      {
        rawgRating: 1.8,
        rawgRatingsCount: 2,
    ...
    if (
      !merged.results.some(
        (result: TitleSummary) => result.id === anticipated.id,
      )
    ) {
      throw new Error(
        "Expected anticipated mainstream title to remain after denoise",
      );
    }
...
Deno.test(
  "Broad franchise denoise keeps low-engagement future mainstream title",
  () => {
    const query = "wolverine";
    const futureDate = new Date(Date.now() + 365 * 24 * 60 * 60 * 1000)
      .toISOString()
    ...
    const junk = createTitleSummary(
      "rawg:junk-future-protection-check",
      "Golfin Wolverine ByMy",
      {
        rawgRating: 1.8,
        rawgRatingsCount: 2,
    ...
    if (
      !merged.results.some(
        (result: TitleSummary) => result.id === anticipatedLowSignal.id,
      )
    ) {
      throw new Error(
        "Expected low-engagement future mainstream title to remain",
      );
    }
  },
);
